...
### Folder contain csv
import argparse 
parser = argparse.ArgumentParser(description="Process initial benchmark results by circuits and combine all the results into new files in all.csv")
...
def exclude():
    ...
    for name in benchmark:
        ...
        df_in = df.query('benchmark_name=={}'.format(name))
        df_ex.to_csv ('{}/allx{}.csv'.format(path, name), index=False) 
        df_in.to_csv ('{}/{}.csv'.format(path, name), index=False) 
